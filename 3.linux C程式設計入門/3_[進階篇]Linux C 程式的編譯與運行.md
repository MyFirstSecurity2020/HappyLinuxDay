...
Program Execution Stages 執行的各階段(作業系統 如何載入執行檔 使其執行)

編譯的各階段 ==> 參考底下延伸閱讀的資料
 預處理|前置處理 (preprocess)===> 原始程式檔hello.c 和相關的標頭檔(如stdio.h)等被前編譯器cpp 編譯成一個.i 檔案
...
4.刪除所有的註解"//"和"／*    *／" 
5.添加行號和檔案名稱標誌，比如＃2 “hello.c” 2 ，以便於編譯時編譯器產生除錯用的行號資訊，及用於編譯時產生編譯錯誤或警告時能揖顯示行號
