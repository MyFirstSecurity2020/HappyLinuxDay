...
```
## 完成底下幾個  逆向  成  組合語言程式  的  練習{把-S 改成-D 再做一次}
```
objdump -S  objd2 ==> 會產生AT&T格式 的 組合語言程式

objdump -S -M intel objd2 ==> 會產生INTEL 格式 的 組合語言程式

objdump -S  -j .text -M intel objd2 ==> 只產生 程式區塊(.text)  INTEL 格式 的 組合語言程式
...
Disassembly of section .text: <== 程式區塊 

0000000000001050 <_start>:   <== 程式區塊 起始點
    1050:	31 ed                	xor    ebp,ebp  <== xor->組合語言指令  ebp->暫存器(register)
...
0000000000001135 <main>:  <== c程式的main() 
// objd2.c
#include <stdio.h>
